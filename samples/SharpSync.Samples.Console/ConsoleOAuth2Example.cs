...
public class ConsoleOAuth2Provider: IOAuth2Provider {
    ...
    public async Task<OAuth2Result> AuthenticateAsync(
        ...
        System.Console.WriteLine($"If the browser doesn't open, navigate to:\n{authUrl}\n");
        OpenBrowser(authUrl);

        // 4. Wait for the OAuth2 callback
        System.Console.WriteLine("Waiting for authorization callback...");
...
public static class OAuth2SyncExample {
    public static async Task RunAsync() {
        ...
        System.Console.WriteLine($"Auth URL: {oauthConfig.AuthorizeUrl}");
        System.Console.WriteLine($"Token URL: {oauthConfig.TokenUrl}");

        // 2. Create the OAuth2 provider and authenticate
        var oauthProvider = new ConsoleOAuth2Provider();
        System.Console.WriteLine("Starting OAuth2 authentication...");
        var authResult = await oauthProvider.AuthenticateAsync(oauthConfig);
        System.Console.WriteLine($"Authenticated as: {authResult.UserId ?? "unknown"}");
        System.Console.WriteLine($"Token expires at: {authResult.ExpiresAt:u}");
        ...
        var resolver = new SmartConflictResolver(
            conflictHandler: (analysis, ct) => {
                System.Console.WriteLine($"Conflict on: {analysis.FilePath}");
        ...
        engine.FileProgressChanged += (s, e) =>
            System.Console.WriteLine($"  {e.Operation}: {e.Path} — {e.PercentComplete}%");
        ...
        if (plan.HasChanges) {
            System.Console.Write("Proceed with sync? [y/N] ");
            if (System.Console.ReadLine()?.Trim().ToLowerInvariant() == "y") {
                var result = await engine.SynchronizeAsync();
                System.Console.WriteLine($"Sync complete: {result.FilesSynchronized} files synced");
            }
        } else {
            System.Console.WriteLine("Everything is up to date.");
        }
        ...
        if (authResult.WillExpireWithin(TimeSpan.FromMinutes(5))
            ...
            System.Console.WriteLine("Token expiring soon, refreshing...");
            var newResult = await oauthProvider.RefreshTokenAsync(
                oauthConfig, authResult.RefreshToken);
            System.Console.WriteLine($"Token refreshed, new expiry: {newResult.ExpiresAt:u}");
        }
    }
}
